...
class Entity_Linking_CNN(nn.Module):
    ...
    def forward(self, index_candidates, index_mentions, index_sentence, index_summary, index_mentions_word, index_candidates_word):
        ...
        features = torch.cat(( feature_sentence, feature_mlp, feature_dot, feature_sub, feature_sum, feature_summary, score_cosin), dim=-1)
        # features = torch.cat((feature_sentence, feature_mlp, feature_summary), dim=-1)
