...
if __name__=='__main__':
    ...
    train_dataset = CustomerDataset(train_samples, sentences, summary, num_mentions, num_candidates, max_length_seq_char, max_length_seq_sence, max_length_word, char2id, word2id, candidate_prob, candidate_elsearch, device=device)
    valid_dataset = CustomerDataset(valid_samples, sentences, summary, num_mentions, num_candidates, max_length_seq_char, max_length_seq_sence, max_length_word, char2id, word2id, candidate_prob, candidate_elsearch, device=device)
    test_dataset = CustomerDataset(samples_test, sentences_test, summary, num_mentions, num_candidates, max_length_seq_char, max_length_seq_sence, max_length_word, char2id, word2id, candidate_prob, candidate_elsearch, device=device)
    ...
    print("Load model ....")
    model.load_state_dict(torch.load('../data/model_el.pt'))
    print("Complete load...")
    ...
    for e in range(epoch):
        ...
        if acc_valid > best:
            best = acc_valid
            torch.save(model.state_dict(), '../data/model_el.pt')
