...
from model_el import Entity_Linking, Entity_Linking_Base
...
def load_data():
    ...
    ## Load samples
    with open('../data/samples_1500000.pkl', 'rb') as f:
...
def train(clip=1):
    ...
    for batch in tqdm(train_dataloader):
        ...
        score_candidate = model(index_candidates, index_mentions, index_sentence)
        ...
        with torch.no_grad():
            if iters % 1000 == 0:
                print(f"Accuracy train = {accuracy_score(labels, predicts)}, Loss train = {loss_epoch}")
        iters += 1
...
if __name__=='__main__':
    ...
    ## argument mlp score
    input_dim_mlp_score, output_dim_mlp_score, num_hidden_mlp_score1, num_hidden_mlp_score2 = output_dim_mlp_embed + hidden_size * 2 + 1, 1, 256, 10
    ...
    train_samples = samples[:1000000]
    valid_samples = samples[1000000:1250000]

    # train_samples = samples[:10000]
    # valid_samples = samples[10000:12500]
    ...
    model = Entity_Linking_Base(char_embed, sentence_embed, mlp_embed, mlp_score, num_mentions, num_candidates, max_length_seq_char, max_length_seq_sence, max_length_word, dim_char, device)
    loss = MyMarginLoss()
    ...
    # print("Load model ....")
    # model.load_state_dict(torch.load('../data/model_el_3000000.pt'))
    # print("Complete load...")
    ...
    for e in range(epoch):
        ...
        if acc_valid > best:
            best = acc_valid
            torch.save(model.state_dict(), './model/model_el_base.pt')
